      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className="max-w-xl mx-auto">
          <div className="text-center mb-10">
            ...
            <h1 className="text-4xl font-bold text-chabad-brown mb-3 font-serif">Make a Donation</h1>
            <p className="text-chabad-text-mid">
              Your generosity lights up our community. Every contribution — large or small — makes a difference.
            </p>
          </div>

          {state === 'success' ? (
            <div className="bg-white border border-chabad-brown/15 rounded-2xl p-10 text-center shadow-sm">
              <div className="text-chabad-amber text-6xl mb-4">✦</div>
              <h2 className="text-2xl font-bold text-chabad-brown mb-3 font-serif">Thank You!</h2>
              <p className="text-chabad-text-mid mb-3 text-lg">{message}</p>
              {chargeId && (
                <p className="text-xs text-chabad-text-muted mb-2">PayArc Charge ID: {chargeId}</p>
              )}
              <p className="text-xs text-chabad-text-muted mb-6">
              ...
              <button onClick={() => { setState('idle'); setChargeId('') }}
                className="text-sm text-chabad-amber hover:text-chabad-brown underline">
                Make another donation
              </button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}
              className="bg-white border border-chabad-brown/15 rounded-2xl p-8 space-y-6 shadow-sm">

              {/* Donation type */}
              <div className="flex rounded-lg overflow-hidden border border-chabad-brown/20">
                {(['one-time', 'recurring'] as const).map(t => (
                  <button key={t} type="button" onClick={() => setType(t)}
                    className={`flex-1 py-2.5 text-sm font-bold transition ${
                      type === t
                        ? 'bg-chabad-brown text-white'
                        : 'bg-transparent text-chabad-text-mid hover:text-chabad-brown'
                    }`}>
              ...
              {/* Amount presets */}
              <div>
                <label className="text-xs text-chabad-text-muted uppercase tracking-wider block mb-3">Amount</label>
                    ...
                    <button key={a} type="button"
                      ...
                      className={`py-2.5 rounded text-sm font-bold transition ${
                        amount === a && !customAmount
                          ...
                          : 'border border-chabad-brown/20 text-chabad-text-mid hover:border-chabad-amber hover:text-chabad-brown'
                      }`}>
                      ${a}
                    </button>
                  ))}
                ...
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-chabad-text-muted">$</span>
                  <input
                    type="number" min="1" placeholder="Custom amount"
                    value={customAmount}
                    onChange={e => { setCustom(e.target.value); setAmount(0) }}
                    className="w-full bg-white border border-chabad-brown/20 rounded pl-7 pr-3 py-2 text-chabad-text text-sm focus:border-chabad-amber outline-none"
                  />
                </div>
              </div>
              ...
              {/* Designation */}
              <div>
                <label className="text-xs text-chabad-text-muted uppercase tracking-wider block mb-2">Designation</label>
              ...
              {/* Personal info */}
              <div>
                <label className="text-xs text-chabad-text-muted uppercase tracking-wider block mb-3">Your Information</label>
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <input required placeholder="First name" value={form.firstName}
                      onChange={e => setForm(f => ({...f, firstName: e.target.value}))}
                      className="bg-white border border-chabad-brown/20 rounded px-3 py-2 text-chabad-text text-sm focus:border-chabad-amber outline-none" />
              ...
              <div>
                <label className="text-xs text-chabad-text-muted uppercase tracking-wider block mb-3">
                ...
                <div className="space-y-3">
                  ...
                  <input placeholder="Card number" value={card.number}
                    ...
                    className="w-full bg-white border border-chabad-brown/20 rounded px-3 py-2 text-chabad-text text-sm focus:border-chabad-amber outline-none font-mono tracking-widest"
                    maxLength={19} />
                ...
                <p className="text-xs text-chabad-text-muted mt-2">
              ...
              <div className="bg-chabad-ltgold rounded-lg p-4 border border-chabad-amber/20">
                <div className="flex justify-between text-sm">
                  <span className="text-chabad-text-mid">{type === 'recurring' ? 'Monthly donation' : 'One-time donation'}</span>
                  <span className="text-chabad-brown font-bold text-lg">${finalAmount || 0}</span>
                </div>
                <div className="text-xs text-chabad-text-muted mt-1">{designation}</div>
              </div>
              ...
              <button type="submit" disabled={state === 'loading' || !finalAmount}
                className="w-full py-4 bg-chabad-brown text-white font-bold text-lg rounded hover:bg-chabad-dark disabled:opacity-50 transition">
                {state === 'loading'
                  ? 'Processing...'
